...
- Can be directly saved as `lesson.html` and opened in a browser to run perfectly
- Supports mobile touch

### 3. Page Structure

#### Top Navigation Bar
- Large title on the left (topic name + bilingual)
- Buttons on the right: "Reset", "Random Experiment", "Fullscreen", "About"
- Background: `--nav-bg`
...
- Default position: right side area
- Must have a "Hide" button (✕ or icon) in top-right corner
- When hidden, displays as a **floating circular button in the bottom-right corner** (with quiz icon)
- Clicking the floating button re-expands the panel
- Panel expand/collapse with smooth transition animation (transition: all 0.3s ease)
- Panel dimensions: width 360px, max height 380px
...
- Uses projectVector for synchronization
...
- Cyber-education aesthetic / Glassmorphism + neon accent colors
...
- Instant feedback at every interaction point (Toast notifications + highlighted explanations + 3D highlights)

### Features

- Include a "Reset to Initial State" button
- Include a "Random Experiment" button
...
2. **Auto-Detection Analysis**
   - **Subject Recognition**: Identify the subject domain based on keywords (Physics/Chemistry/Biology/Math/Astronomy/Programming)
   - **Rendering Scheme Detection**: Determine whether to use Three.js pure 3D / SVG 2D / Hybrid mode based on topic characteristics
...
3. **Generate HTML**
   ...
   - Determine whether to include SVG/D3.js based on rendering mode
